...
global de citações. Aqui, a proposta é um algoritmo capaz de empregar a engenhosidade da contagem de citações de PageRank na 
computação de confiança. Perceba que a rede de confiança da comunidade acadêmica gera uma matriz simétrica: Um voto de confiança 
através de colaboração em publicações é mútuo entre os autores, logo, há propensão ao problema do \textit{rank sink} e que o 
conceito de um \textit{random surfer} não faz sentido pois não se trata de um modelo de navegação em  páginas da \textit{web}. 
Portanto, a proposta é a introdução de heurísticas capazes de aperfeiçoar a aplicação do algoritmo, adaptando o problema ao 
modelo proposto: Propagação de confiança em uma rede de colaborações. 

\textbf{Paragrafo sobre computar confiança local}
