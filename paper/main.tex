...
O algoritmo PageRank foi inspirado em parte em estudos realizados em redes de citações acadêmicas, nas quais a relevância de um 
artigo era descrita por contagem de citações, por exemplo. Trata-se de um método para computar um ranking global de citações, 
pensado para computar a importancia das paginas web. O ranking $R$ de uma página é definido como a soma dos rankings das páginas 
que oferecem links para ela. É definido para cada página $u$ um conjunto $F_u$ de páginas as quais $u$ referencia e um conjunto 
$B_u$ de páginas que fazem referência à $u$. Considerando uma matriz de adjacência $A_{u,v}$ na qual o valor é $1/N_u$ nas 
posições onde há referência de $u$ para $v$, PageRank pode ser definido como $R = c(AR + E)$, sendo $c$ um fator de normalização.
\cite{page1999pagerank}
...
seria aproximadamente igual ao número de colaborações (publicações em conjunto). Enquanto esta métrica é relevante, perde-se a 
idéia inicial: Não é considerada a confiança dos colaboradores, somente o valor total de colaborações. Além disso, alguns conceitos 
...
Expertise é o conceito de quanto conhecimento determinado pesquisador possui em sua área de atuação. Para estabelecer uma 
heurística de expertise, é usado um modelo semelhante ao usado para estimar a relevância de  um artigo: Para cada ponto 
considerado relevante à expertise do autor, é atribuído um valor e então a heurística é o somatório dos valores dos campos 
que possuem dados.

\textbf{** tabela **}

\subsubsection{PageRank Heurístico}

\textbf{Explicar como são usadas as heuristicas e o que significa para a aplicação do pagerank}

\subsubsection{Segunda métrica de propagação  de confiança (local)}

\textbf{Explicar o algoritmo para computar a cofiança local (alternativa ao pagerank)}

\subsubsection{Algoritmo baseado em conteudo}

\textbf{Apos computar a confiança, apliar um algoritmo baseado em conteudo simples para selecionar artigos para recomendação.
a seleção será ordenada conforme a confiança.}
