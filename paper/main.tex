...
\subsubsection{The PageRank Citation Ranking: Bringing Order to the Web}
...
Para chegar nas recomendações, é proposto um trabalho em dois momentos: Estimar a confiança da comunidade em cada pesquisador 
e então selecionar publicações baseando-se no perfil dos pesquisadores. A seleção será ordenada de acordo com o nìvel de  
confiança estimado das publicações. O primeiro passo é modelar uma \textbf{rede de confiança} da comunidade científica, 
descrita por autores e publicações. Pode então ser feita uma seleção das publicações cadastradas com base no perfil do usuário 
e ordenar a seleção de acordo com o nível de confiança de cada item. Em termos de ferramentas para implementação, foi escolhida 
a linguagem python, que possui riqueza de ferramentas e recursos para trabalhos relacionados a manipulação de dados e computação 
numérica.% \cite{python book}
...
Uma publicação científica é uma produção de alguma \textbf{natureza} (artigo, livro, trabalho) que passou por um processo de 
...
nome, e o \textbf{orientador} do trabalho o último. Entre eles, os colaboradores \textbf{intermedários}.  
...
pode ser usada para computar a propagação de confiança através da rede. A confiança da comunidade em uma determinada publicação 
é a soma da confiança investida pela comunidade em cada pesquisador envolvido no trabalho. O algoritmo PageRank foi inspirado em 
parte em estudos realizados em redes de citações acadêmicas, nas quais a relevância de um artigo era descrita por contagem de 
citações, por exemplo \cite{page1999pagerank}. Aqui, a proposta é um algoritmo capaz de empregar a engenhosidade da contagem de 
citações de PageRank na computação de confiança. Fica definido como escore de confiança de um autor a estimativa de confiança 
investida nele pela comunidade, conforme será descrito a seguir.

Perceba que a rede de confiança da comunidade acadêmica gera uma matriz simétrica, que seria propensa ao problema do 
\textit{rank sink} e que o conceito de um \textit{random surfer} não faz sentido pois não se trata de um modelo de navegação em  
páginas da \textit{web}. Portanto, a proposta é a introdução de heurísticas capazes de aperfeiçoar a aplicação do algoritmo, 
adaptando o problema ao modelo proposto: Propagação de confiança em uma rede de colaborações. 
...
usada na base de conhecimento para construir uma heurśtica sobre a relevância da publicação. Um valor é atribuído a cada uma 
das características relevantes da publicação, quando presentes e a heurística é a soma das pontuações. A pontuação  é 
considerada conforme a tabela.
