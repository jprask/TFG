...
que oferecem links para ela, ponderados pelo número total de links saindo das páginas. É definido para cada página $u$ um conjunto 
$F_u$ de páginas as quais $u$ referencia e um conjunto $B_u$ de páginas que fazem referência à $u$. Considerando uma matriz de 
adjacência $A_{u,v}$ na qual o valor é $^1/_{N_u}$ nas posições onde há referência de $u$ para $v$, sendo $N_u$ a soma de links 
saindo de $u$. PageRank pode ser definido como $R = c(AR + E)$, sendo $c$ um fator de normalização \cite{page1999pagerank}.
